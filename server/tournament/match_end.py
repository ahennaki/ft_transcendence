...
from tournament.serializers     import TournamentParticipantSerializer

async def end_match(consumer, isDisconnect):
    print_green(f'end of the game!')
    wn, ln, winner, loser, ws, ls = await determine_winner_loser(consumer, isDisconnect)
    print_green(f'winner: {winner}')

    await save_tournament_match(consumer, winner, loser, ws, ls)
    # await set_badge(winner, 1)
    # await set_badge(loser, 0)

    winner_participant = await get_participant_data(winner)
    loser_participant = await get_participant_data(loser)
    match_round = 0
    ...
    data = {"type": "end_game", "match_id": consumer.match_id, "winner": wn,
        ...
        "loser_participant": loser_participant,
...
@database_sync_to_async
def get_participant_data(player):
    serializer = TournamentParticipantSerializer(player)  
...
@database_sync_to_async
def save_tournament_match(consumer, winner, loser, winner_score, loser_score):
    match = TournamentMatch.objects.get(id=consumer.match_id)

    match_history, created = TournamentMatchHistory.objects.update_or_create(
